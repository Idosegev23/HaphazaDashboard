...
export default function CreatorCatalogPage() {
  ...
  const loadCreators = async () => {
    ...
    setCreators((data || []) as unknown as CatalogCreator[]);
    setLoading(false);
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-[#212529] text-xl">טוען קטלוג יוצרים...</div>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <input
              ...
              className="px-4 py-2 bg-[#f8f9fa] border border-[#dee2e6] rounded-lg text-[#212529] focus:outline-none focus:border-[#f2cc0d] w-48"
            />
            {/* Favorites toggle */}
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-colors border ${
                showFavoritesOnly
                  ...
                  : 'bg-[#f8f9fa] text-[#212529] border-[#dee2e6] hover:bg-[#e9ecef]'
              }`}
            >
              {showFavoritesOnly ? '\u2764\uFE0F מועדפים' : '\u2661 מועדפים'}
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-colors border ${
                hasActiveFilters && !showFavoritesOnly
                  ? 'bg-[#f2cc0d] text-black border-[#f2cc0d]'
                  : 'bg-[#f8f9fa] text-[#212529] border-[#dee2e6] hover:bg-[#e9ecef]'
              }`}
            >
        ...
        {filteredCreators.length > 0 ? (
          ...
          <Card>
            <p className="text-[#6c757d] text-center py-12">
              {showFavoritesOnly
                ? 'אין יוצרים מועדפים עדיין. לחץ על הלב בכרטיס יוצר כדי להוסיף למועדפים.'
                : hasActiveFilters
                ? 'לא נמצאו יוצרים התואמים את הפילטרים. נסה לשנות את הסינון.'
        ...
        {sc && (
          <div className="space-y-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {sc.niches.map((niche) => (
                    <span
                      key={niche}
                      className="px-3 py-1 bg-[#f8f9fa] rounded-full text-sm text-[#212529] border border-[#dee2e6]"
                    >
                      {niche}
                    </span>
                  ))}
            ...
            <div>
              ...
              {loadingPortfolio ? (
                <p className="text-[#6c757d] text-sm">טוען...</p>
              ) : portfolioItems.length > 0 ? (
                <div className="grid grid-cols-2 gap-3">
                  {portfolioItems.map((item) => (
                    <div
                      key={item.id}
                      className="aspect-square rounded-lg overflow-hidden bg-[#f8f9fa] border border-[#dee2e6]"
                    >
                ...
                <p className="text-[#6c757d] text-sm">אין פריטים בתיק העבודות</p>
              )}
            </div>
          </div>
        )}
