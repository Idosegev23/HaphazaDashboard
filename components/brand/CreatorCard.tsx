...
export function CreatorCard({ creator, onClick, isFavorite, onToggleFavorite }: CreatorCardProps) {
  ...
  return (
    <div
      className="group relative rounded-2xl overflow-hidden bg-white cursor-pointer hover:shadow-lg transition-all duration-300"
      style={{ height: 370 }}
      onClick={onClick}
    >
      {/* === HERO IMAGE === */}
      <div className="relative flex-1 overflow-hidden rounded-t-2xl" style={{ height: 250 }}>
        {heroItem ? (
        ...
        {onToggleFavorite && (
          <button
            ...
            className="absolute top-2.5 right-2.5 z-10 w-[30px] h-[30px] rounded-full bg-[#333]/70 flex items-center justify-center transition-all hover:bg-[#333]/90 hover:scale-110"
        ...
        <div className="absolute bottom-2.5 right-2.5 z-10 flex items-center gap-2">
          <span className="text-white text-base font-normal drop-shadow-sm truncate max-w-[140px]">
            {displayName}
          </span>
          <div className="w-[30px] h-[30px] rounded-full overflow-hidden flex-shrink-0 bg-[#8e33f5]">
            {profile?.avatar_url ? (
      ...
      <div className="px-2.5 py-3.5 space-y-2">
        ...
        <div className="flex items-center justify-between">
          <span className="text-sm font-semibold text-[#6b7281]">
          ...
          <div className="flex items-center gap-1.5">
            {platformEntries.slice(0, 2).map(([name, data]) => (
              <span
                key={name}
                className="inline-flex items-center gap-1.5 bg-[#eee] border border-[#dfdfdf] rounded-full px-2 py-1"
              >
                <span className="text-[14px] text-[#6b7281]">
        ...
        {creator.niches && creator.niches.length > 0 && (
          <div className="flex justify-end">
            <span className="inline-flex items-center justify-center bg-[#ffd485] border border-[#ffc65c] rounded-full px-2 h-[22px] text-[13px] text-[#6b7281]">
              {creator.niches[0]}
            </span>
          </div>
        )}
        ...
        {(creator.city || creator.country) && (
          <div className="flex justify-end">
            <span className="text-sm text-[#6b7281] truncate">
